...
export default function DetalleSemana({ route, navigation }) {
  ...
  // ⏱️ Tiempo persistente
  const [tiempo, setTiempo] = useState(0);
  const keyTiempo = `tiempo-${contenido.semana}`;
  const keyUltimaEntrada = `ultimaEntrada-${contenido.semana}`;

  useEffect(() => {
    const cargarTiempo = async () => {
      try {
        const tiempoGuardado = await AsyncStorage.getItem(keyTiempo);
        const ultimaEntrada = await AsyncStorage.getItem(keyUltimaEntrada);

        let acumulado = tiempoGuardado ? parseInt(tiempoGuardado, 10) : 0;

        if (ultimaEntrada) {
          const ahora = Date.now();
          const diffSegundos = Math.floor(
            (ahora - parseInt(ultimaEntrada, 10)) / 1000
          );
          acumulado += diffSegundos;
        }

        setTiempo(acumulado);

        // Guardar nueva entrada
        await AsyncStorage.setItem(keyUltimaEntrada, Date.now().toString());
        await AsyncStorage.setItem(keyTiempo, acumulado.toString());
      } catch (error) {
    ...
    const interval = setInterval(async () => {
      setTiempo((prev) => {
        ...
        AsyncStorage.setItem(keyTiempo, nuevoTiempo.toString());
        return nuevoTiempo;
      });
    }, 1000);
  ...
  // ⏱️ Tiempo mínimo según semana
  const tiempoMinimo = contenido.semana === 6 ? 720 : 600; // 12 min = 720 s, 10 min = 600 s

  if (tiempo < tiempoMinimo) {
    alert(
      `Debes permanecer al menos ${
        Math.floor(tiempoMinimo / 60)
      } minutos en esta semana.`
    );
    return;
  }

  if (modulosVistos.length !== contenido.recursos.length) {
    alert("Debes completar todos los módulos antes de continuar.");
    return;
  }
        ...
        <Text style={styles.botonTexto}>
          {completado
            ...
            : `Marcar como completado ${
                tiempo < 600
                  ? `(faltan ${Math.floor((600 - tiempo) / 60)}m ${
                      (600 - tiempo) % 60
                    }s)`
                  : modulosVistos.length !== contenido.recursos.length
                  ? `(faltan ${
                      contenido.recursos.length - modulosVistos.length
                    } módulos)`
                  : ""
              }`}
        </Text>
